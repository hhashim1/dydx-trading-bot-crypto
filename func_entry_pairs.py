from constants import ZSCORE_THRES, USD_PER_TRADE, USD_MIN_COLLATERAL
...
def open_positions(client):
    ...
    for index, row in df.iterrows():
        ...
        if len(series_1)  > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRES:

                ## Ensure life-for-like not alredy open (diversify trading)
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    if check_base and check_quote:
                        ...
                        free_collatteral = float(account.data["account"]["freeCollateral"])
                        print(f"Balance: {free_collatteral} and minimum at {USD_MIN_COLLATERAL}")

                        # Guard: Ensure collateral
                        if free_collatteral < USD_MIN_COLLATERAL:
                            break
